...
for sex in sex_list:
    ...
    if sex == "male":
        X = X[X["sex12"] == 1]
        X.drop(columns="sex12", axis=1, inplace=True)
    else:
        X = X[X["sex12"] == 2]
        X.drop(columns="sex12", axis=1, inplace=True)
    
    # Filter y and groups to match the index of X after sex selection
    y = y_original.loc[X.index]
    groups = groups_original.loc[X.index]
...
# %%
    ...
    for repeat in range(n_repeats):
        ...
        cv_train_predictions = np.zeros(
            len(y)
        )  # For storing cross-validated predictions
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(
            ...
            X_train_transformed = best_model.named_steps["preprocessor"].transform(
                X_train
            )
    ...
    # Save results (add sex to the filename)
    np.save(save_dir / f"train_mae_{exp_name}_{sex}.npy", outer_train_mae)
    np.save(save_dir / f"train_r2_{exp_name}_{sex}.npy", outer_train_r2)
    np.save(save_dir / f"train_corr_{exp_name}_{sex}.npy", outer_train_corr)
    np.save(
        save_dir / f"train_full_predictions_{exp_name}_{sex}.npy",
        train_full_predictions,
    )

    np.save(save_dir / f"mae_{exp_name}_{sex}.npy", outer_mae)
    np.save(save_dir / f"r2_{exp_name}_{sex}.npy", outer_r2)
    np.save(save_dir / f"corr_{exp_name}_{sex}.npy", outer_corr)
    np.save(save_dir / f"full_predictions_{exp_name}_{sex}.npy", full_predictions)
    np.save(save_dir / f"full_y_true_{exp_name}_{sex}.npy", full_y)

    with open(save_dir / f"shap_explanations_{exp_name}_{sex}.pkl", "wb") as f:
    ...
    all_feature_importances_df.to_csv(
        save_dir / f"all_feature_importances_{exp_name}_{sex}.csv", index=False
    )

    pd.DataFrame({"feature": feature_names}).to_csv(
        save_dir / f"final_feature_names_{exp_name}_{sex}.csv", index=False
    )

summary = {
    "script_name": "experiment_using_voice_features_by_sex.py",
    "n_repeats": n_repeats,
    "outer_cv_folds": n_outer_folds,
