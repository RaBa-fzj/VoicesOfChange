...
for repeat in range(n_repeats):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(
        ...
        cv_gender[test_idx] = X_test["sex12"].values
        if verbose:
            print(f"  MAE: {mae:.3f}")
            print(f"  R²: {r2:.3f}")
    ...
    gender_loop.append(cv_gender.copy())
...
full_gender = np.concatenate(gender_loop)
...
np.save(save_dir / f"gender_{exp_name}.npy", full_gender)
